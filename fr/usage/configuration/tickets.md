---
description: >-
  Ici, vous trouverez des informations sur la configuration du système de ticket de John-Bot.
---
...
# :earth_africa: Configuration globale
...
{% hint style="info" %} Il est fortement conseillé de rendre le salon des logs de ticket privé. En effet, certaines informations pourraient s'avérer privées et ne doivent donc pas être accessibles au premier venu mais à l'équipe de confiance. {% endhint %}

## Limite de tickets globale
Cette option permet de limiter le nombre de tickets total ouverts simultanément par un membre sur le serveur, tous panneaux confondus. Si une limite de ticket est appliquée plus tard à l'un des panneaux, cette dernière prendra le dessus pour le panneau en question.
Choisissez simplement le nombre de tickets maximum avec le champ correspondant.
...
{% hint style="info" %} Si vous ne trouvez pas la transcription que vous cherchez, c'est qu'elle n'est plus stockée car le message de logs du ticket en question a été supprimé ou que votre ticket remonte avant le 26 janvier 2024. Dans la première situation, rien à faire, votre transcription est définitivement effacée. Dans la seconde situation, il suffit de chercher le message de logs associé au ticket souhaité et de télécharger le fichier .html joint au message et de l'ouvrir dans votre visionneur de fichiers .html. Les navigateurs web (Mozilla Firefox, Microsoft Edge, Opera, Google Chrome...) supporte généralement ce type de fichiers. {% endhint %}

## Rôle·s d'exclusion
Les rôles d'exclusion permettent d'empêcher certains membres d'ouvrir des tickets. Lorsqu'un membre possède un rôle d'exclusion, il sera impossible pour lui de créer un ticket.
Choisissez un ou plusieurs rôles parmi votre liste de rôles dans le champ correspondant pour configurer cette option.
...
<br/> Une fois les configurations terminées, il vous suffit de cliquer sur le bouton vert `Enregistrer` situé en bas à droite de la fenêtre de configuration. Si vous ne souhaitez pas conserver vos modifications, cliquez simplement sur `Annuler`.
...
{% hint style="info" %} Pour apprendre à configurer un modèle, consulter la catégorie de la documentation correspondante, sur cette page : [Configuration modèle·s]\(../../usage/configuration/tickets.md#bookmark_tabs-configuration-modèles) {% endhint %}
...
<br/> Pour définir la période avant l'envoi du rappel d'inactivité, séléctionnez l'option qui vous convient le mieux dans le menu de séléction disponible dans le champ.

{% hint style="info" %} Cette option n'est affective que lorsque l'option `Activer le rappel d'inactivité` est activée. Pour apprendre à l'activer, consultez cette catégorie, sur cette page : [Activer le rappel d'inactivité]\(../../usage/configuration/tickets.md#rappel-dinactivité) {% endhint %}
...
<br/> Si cette limite est définie, elle prend le dessus sur la limite globale définie préalablement.
...
{% hint style="info" %} Cette option permet d'utiliser l'option de `Gestion exclusive du ticket`. Pour apprendre à l'activer, consultez cette catégorie, sur cette page : [Gestion exclusive du ticket]\(../../usage/configuration/tickets.md#gestion-exclusive-du-ticket) {% endhint %}
...
{% hint style="info" %} Pour savoir comment définir le temps qui doit s'écouler avant l'envoi du rappel, consultez cette catégorie sur cette page : [Gestion exclusive du ticket]\(../../usage/configuration/tickets.md#période-avant-lenvoi-du-rappel-dinactivité-gem) {% endhint %}

![Exemple de message de rappel](../../.gitbook/assets/ticket_rappel_example.png)
...
![Exemple d'avertissement en message privé](../../.gitbook/assets/ticket_avertissement%20privé_example.png)
...
![Exemple de message d'ouverture avec le message texte mis en évidence](../../.gitbook/assets/ticket_message%20d'ouverture_partie%20texte.png)
...
![Exemple de message d'ouverture avec l'embed mis en évidence](../../.gitbook/assets/ticket_message%20d'ouverture_partie%20embed.png)
...
## Configurer un embed
Une fois dans les paramètres d'un embed, vous pouvez configurer de nombreuses options pour personnaliser le système selon vos besoins.
